...
class Widget extends Void
{
	 ...
	 * @var string
	 */
	protected $componentClass = 'widget';
	...
	{
		if ( empty($this->getComponentTagname()) ) {
			return '';
		}
		$componentOpen = '<'.$this->getComponentTagname().''.( empty($this->getComponentClass()) ? '' : ' class="'.$this->getComponentClass().'"').'>';
		$componentClose = '</'.$this->getComponentTagname().'>';
		$headerOpen = $headerClose = $header = '';
		if ( !empty($this->getHeaderTagname()) ) {
			$headerOpen = '<'.$this->getHeaderTagname().''.( empty($this->getHeaderClass()) ? '' : ' class="'.$this->getHeaderClass().'"').'>';
			$headerClose = '</'.$this->getHeaderTagname().'>';
		}
		$footerOpen = $footerClose = $footer = '';
		if ( !empty($this->getFooterTagname()) ) {
			$footerOpen = '<'.$this->getFooterTagname().''.( empty($this->getFooterClass()) ? '' : ' class="'.$this->getFooterClass().'"').'>';
			$footerClose = '</'.$this->getFooterTagname().'>';
		}
		$contentOpen = $contentClose = '';
		$content = '<!-- '.__CLASS__.' -->';
		if ( !empty($this->getContentTagname()) ) {
			$contentOpen = '<'.$this->getContentTagname().''.( empty($this->getContentClass()) ? '' : ' class="'.$this->getContentClass().'"').'>';
			$contentClose = '</'.$this->getContentTagname().'>';
		}
		if ( !empty($this->getContent()) ) {
			$content = $this->getContent();
		}
		$html = $componentOpen . PHP_EOL .
			$headerOpen . PHP_EOL .
				$header . PHP_EOL .
			$headerClose . PHP_EOL .
			$contentOpen . PHP_EOL .
				$content . PHP_EOL .
			$contentClose . PHP_EOL .
			$footerOpen . PHP_EOL .
				$footer . PHP_EOL .
			$footerClose . PHP_EOL .
		$componentClose . PHP_EOL;
